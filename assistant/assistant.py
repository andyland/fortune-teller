...
class VoiceAssistant:
    def __init__(self, whisper_url, llm_url, tts_url, samplerate=16000, channels=1, 
                 buffer_duration=20.0, blocksize=1024, system_prompt=None):
        ...
        self.system_prompt = system_prompt or "You are a helpful AI assistant. Provide clear, concise responses."
    ...
    def check_history_timeout(self):
        ...
        if (self.conversation_history and 
            current_time - self.last_activity_time > self.history_timeout):
    ...
    def process_question(self, question: str):
        ...
        try:
            ...
            if len(self.conversation_history) > 0:
                print(f"💬 Conversation context: {len(self.conversation_history)} exchange(s) remembered")
            ...
            try:
                ...
                if transcription and len(transcription) > 5:  # Minimum length filter
                    ...
                    if transcription != self.last_transcription:
                        ...
                        print(f"\r🎙️ Listening: {transcription[:50]}{'...' if len(transcription) > 50 else ''}", end="", flush=True)
                    
                    # Check for silence (transcription hasn't changed for a while)
                    elif (current_time - self.last_transcription_time > self.silence_threshold and 
                          not self.processing and len(transcription.strip()) > 10):
